...
onMounted(() => {
  ...
  observer = Observer.create({ target: window, type: 'wheel,touch,keydown', wheelSpeed: -1, tolerance: 14, preventDefault: true, onDown: () => goToScene(currentIndex.value + 1, 'down'), onUp: () => goToScene(currentIndex.value - 1, 'up'), onKeyDown: (event) => { if (['ArrowDown', 'PageDown', 'ArrowLeft'].includes(event.key)) goToScene(currentIndex.value + 1, 'down'); if (['ArrowUp', 'PageUp', 'ArrowRight'].includes(event.key)) goToScene(currentIndex.value - 1, 'up') } })
})
